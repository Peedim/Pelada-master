...
// ... (Funções auxiliares calculateTeamStats e mapDatabaseToMatch mantidas iguais - omitidas para brevidade) ...

export const matchService = {
  // ... (Outras funções getAll, getById, createDraft etc mantidas iguais) ...
  // Vou focar apenas no finishMatch que foi alterado:
  ...
  finishMatch: async (matchId: string): Promise<void> => {
    ...
    match.teams.forEach(team => {
        ...
        teamGames.forEach(game => {
            ...
            // Desempate por pênaltis conta como vitória para stats? Geralmente sim.
    ...
    // APLICAÇÃO DOS MODIFICADORES (TABELA CSV)
    for (const team of match.teams) {
        for (const player of team.players) {
            ...
            // 1. RITMO (Geral para todos)
            // Tabela: Vitória = +0.3, Derrota = -0.2
            dPace += (s.wins * 0.3);
            dPace += (s.losses * -0.2);
            // Nota: "Jogar a Partida" (+0.2) foi removido conforme discussão para balancear.
            
            // Bônus Extras de Ritmo
            if (team.id === championId) dPace += 1.0;     // Campeão do Dia
            if (team.id === lastPlaceId) dPace -= 0.5;    // Último Lugar

            // 2. DEFESA (Clean Sheet & Gol Sofrido)
            // Clean Sheet (Escalonado por posição)
            ...
            // Gol Sofrido (Geral -0.2 para todos)
            dDef += (s.goalsConceded * -0.2);

            // 3. FINALIZAÇÃO & PASSE (Gols e Assistências)
            // Gol Marcado (Bonus por posição)
            let golBonus = 0;
            ...
            // 4. PUNIÇÃO: ATAQUE EM BRANCO (Se não fez gol nem assistência no evento todo)
            // Condição da Tabela: "Ataque em branco"
            if (s.goals === 0 && s.assists === 0 && s.matches > 0) {
                if (player.position === PlayerPosition.GOLEIRO || player.position === PlayerPosition.DEFENSOR) {
                    // GK/DEF: -0.1 passe, -0.1 finalização
                    dPass -= 0.1;
                    dShoot -= 0.1;
                } else if (player.position === PlayerPosition.MEIO_CAMPO) {
                    // Meia: -0.2 passe, -0.1 finalização
                    dPass -= 0.2;
                    dShoot -= 0.1;
                } else if (player.position === PlayerPosition.ATACANTE) {
                    // Atacante: -0.2 finalização, -0.1 passe
                    dShoot -= 0.2;
                    dPass -= 0.1;
                }
            }

            // Atualiza acumuladores no banco
  ...
  // ... (calculateStandings, updateChampionPhoto e demais funções auxiliares mantêm-se iguais) ...
  // Inclua aqui o restante do arquivo original (calculateStandings, updateChampionPhoto, getAll, etc.)
  ...
  getAll: async (): Promise<Match[]> => {
    ...
    // @ts-ignore - mapDatabaseToMatch está definida no escopo do arquivo original
  ...
  // ... (Mantenha createDraft, updateMatch, deleteMatch, etc. do seu código original) ...
...
// Funções auxiliares (Necessárias se não estiverem no topo)
