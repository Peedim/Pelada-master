                            ...
                            {games.map(game => {
                                ...
                                <div key={game.id} className={`border p-4 rounded-lg flex items-center justify-between transition-colors ${game.status === GameStatus.FINISHED ? 'bg-slate-900/50 border-slate-800 opacity-75' : 'bg-slate-800 border-slate-700 hover:border-slate-600'}`}>
                                    <div className="flex-1 flex items-center justify-end gap-3">
                                        <span className={`font-bold ${getTeamTextColor(homeName)}`}>{homeName}</span>
                                    ...
                                    <div className="flex-1 flex items-center justify-start gap-3">
                                        ...
                                        <span className={`font-bold ${getTeamTextColor(awayName)}`}>{awayName}</span>
                                    </div>
                    ...
                    <tbody className="divide-y divide-slate-700">
                        {standings.map((team, index) => (
                            <tr key={team.teamId} className="hover:bg-slate-700/30">
                                <td className={`px-4 py-3 font-medium flex items-center gap-2 ${getTeamTextColor(team.teamName)}`}><span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold ${index < 2 ? 'bg-yellow-900/50 text-yellow-500' : 'bg-slate-800 text-slate-500'}`}>{index + 1}</span>{team.teamName}</td>
