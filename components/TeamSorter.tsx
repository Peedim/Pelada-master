...
const TeamSorter: React.FC<TeamSorterProps> = ({ players, onDraftSaved }) => {
  ...
  const toggleLinePlayer = (id: string) => {
    ...
    if (newSet.has(id)) newSet.delete(id); else if (newSet.size < maxLinePlayers) newSet.add(id);
    setSelectedLineIds(newSet);
  };
  const toggleGkPlayer = (id: string) => {
    const newSet = new Set(selectedGkIds);
    if (newSet.has(id)) newSet.delete(id); else if (newSet.size < maxGkPlayers) newSet.add(id);
  ...
  const handleGenerate = () => {
    ...
    if (selectedLineIds.size < (config.type === 'Quadrangular' ? 12 : 9)) { alert("Número insuficiente de jogadores de linha."); setIsGenerating(false); return; }
    setTimeout(() => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-3 space-y-6">
            <div className="bg-slate-800 border border-slate-700 rounded-lg p-4">
                ...
                <div className="space-y-3">
                    <select value={config.type} onChange={(e) => setConfig({ ...config, type: e.target.value as TournamentType })} className="w-full bg-slate-900 border border-slate-700 rounded p-2 text-white text-sm focus:ring-2 focus:ring-blue-500 outline-none"><option value="Quadrangular">Quadrangular (4 times)</option><option value="Triangular">Triangular (3 times)</option></select>
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-lg p-4 flex flex-col h-[400px]">
                <div className="flex justify-between items-center mb-3"><h3 className="text-sm font-bold text-white flex items-center gap-2"><Shirt size={16} className="text-yellow-500" /> Goleiros</h3><span className={`text-xs font-bold px-2 py-0.5 rounded ${selectedGkIds.size === maxGkPlayers ? 'bg-green-900 text-green-400' : 'bg-slate-900 text-slate-400'}`}>{selectedGkIds.size}/{maxGkPlayers}</span></div>
                <div className="flex-1 overflow-y-auto pr-1 space-y-2 custom-scrollbar">
                    {gkPlayersList.map(p => {
                        const isSelected = selectedGkIds.has(p.id);
                        return (<div key={p.id} onClick={() => toggleGkPlayer(p.id)} className={`flex items-center justify-between p-2 rounded cursor-pointer border ${isSelected ? 'bg-slate-700 border-green-500/50' : 'bg-slate-900/50 border-transparent hover:bg-slate-700'}`}><div className="flex items-center gap-2"><div className={`w-3 h-3 rounded-sm border ${isSelected ? 'bg-green-500 border-green-500' : 'border-slate-500'}`}></div><span className={`text-xs font-medium ${isSelected ? 'text-white' : 'text-slate-400'}`}>{p.name}</span></div><span className={`text-xs font-bold ${getOvrColor(p.initial_ovr)}`}>{p.initial_ovr}</span></div>)
                    })}
                </div>
            </div>
        </div>
        ...
        <div className="lg:col-span-5">
            <div className="bg-slate-800 border border-slate-700 rounded-lg p-4 flex flex-col h-[580px]">
                <div className="flex justify-between items-center mb-4"><h3 className="text-lg font-bold text-white flex items-center gap-2"><Users size={20} className="text-blue-400" /> Jogadores de Linha</h3><div className="flex items-center gap-3"><span className={`text-sm font-bold px-3 py-1 rounded-full ${selectedLineIds.size === maxLinePlayers ? 'bg-green-900 text-green-400' : 'bg-slate-900 text-slate-400'}`}>{selectedLineIds.size} / {maxLinePlayers}</span><button onClick={toggleAllLine} className="text-xs text-blue-400 hover:text-white underline">Top {maxLinePlayers}</button></div></div>
                <div className="flex-1 overflow-y-auto pr-2 space-y-2 custom-scrollbar">
                    {linePlayersList.map(p => {
                        ...
                        return (<div key={p.id} onClick={() => !isDisabled && toggleLinePlayer(p.id)} className={`flex items-center justify-between p-3 rounded-lg border transition-all ${isDisabled ? 'opacity-30 cursor-not-allowed' : 'cursor-pointer'} ${isSelected ? 'bg-slate-700/50 border-blue-500/50' : 'bg-slate-900/30 border-slate-800 hover:border-slate-600'}`}><div className="flex items-center gap-3">{isSelected ? <CheckSquare size={18} className="text-blue-500" /> : <Square size={18} className="text-slate-500" />}<div><p className={`text-sm font-medium ${isSelected ? 'text-white' : 'text-slate-400'}`}>{p.name}</p><div className="flex gap-2 text-[10px] text-slate-500"><span>{p.position.substring(0,3)}</span>{p.playStyle && <span className="text-yellow-600 flex items-center gap-0.5"><Zap size={8}/>{p.playStyle}</span>}</div></div></div><span className={`font-bold text-sm ${getOvrColor(p.initial_ovr)}`}>{p.initial_ovr}</span></div>)
                    })}
                </div>
            </div>
