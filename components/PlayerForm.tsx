...
import { supabase } from '../services/supabaseClient'; // Import para checar sessão
...
const PlayerForm: React.FC<PlayerFormProps> = ({ initialData, onSubmit, onCancel, isLoading }) => {
  ...
  // Verifica se o usuário atual é o MASTER (Dono)
  useEffect(() => {
    const checkUserRole = async () => {
      const { data: { user } } = await supabase.auth.getUser();
      const masterEmail = import.meta.env.VITE_MASTER_EMAIL;
      
      // Se o email logado for igual ao do .env, é o Master
  ...
  const currentOvr = useMemo(() => {
    const sum = (Number(pace) || 0) + (Number(shooting) || 0) + (Number(passing) || 0) + 
                (Number(dribbling) || 0) + (Number(defending) || 0) + (Number(physical) || 0);
    ...
    <div className="w-full max-w-4xl mx-auto animate-fade-in">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="grid grid-cols-2 bg-slate-800 p-1 rounded-lg">
          <button type="button" onClick={() => setActiveTab('basic')} className={`py-2 px-4 rounded-md text-sm font-medium transition-all ${activeTab === 'basic' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-white'}`}><div className="flex items-center justify-center gap-2"><User size={16} /> Dados Básicos</div></button>
          <button type="button" onClick={() => setActiveTab('attributes')} className={`py-2 px-4 rounded-md text-sm font-medium transition-all ${activeTab === 'attributes' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400 hover:text-white'}`}><div className="flex items-center justify-center gap-2"><Activity size={16} /> Atributos</div></button>
        </div>
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                  ...
                  <Controller name="playStyle" control={control} render={({ field }) => (<select {...field} className="w-full bg-slate-900 border border-slate-700 rounded-md p-2.5 text-white focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none appearance-none">{Object.values(PlayStyle).map(style => (<option key={style} value={style}>{style}</option>))}</select>)} />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  ...
                  <Controller name="shirt_number" control={control} render={({ field }) => (<input {...field} type="number" className="w-full bg-slate-900 border border-slate-700 rounded-md p-2.5 text-white focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none" />)} />
                </div>
                <div>
              ...
              {/* LÓGICA DO MASTER ADMIN: Só exibe se for o dono */}
              {isMasterAdmin && (
                  ...
                  <Controller
                    ...
                    render={({ field: { value, onChange, ...rest } }) => (
                      <div className="flex items-center h-6">
                        <input id="is_admin" type="checkbox" checked={value} onChange={onChange} {...rest} className="w-5 h-5 rounded border-slate-600 bg-slate-800 text-purple-600 focus:ring-purple-500 focus:ring-offset-slate-900" />
                      </div>
                    )}
                  />
                  <div>
                    <label htmlFor="is_admin" className="text-sm font-bold text-white flex items-center gap-2">
                      <Shield size={14} className="text-purple-400" />
                      Dar permissões de Administrador
                    </label>
                    <p className="text-xs text-slate-400 mt-1">
                      (Visível apenas para o Master) Permite que este jogador gerencie eventos.
                    </p>
                  </div>
                </div>
              )}
          ...
          <div className={activeTab === 'attributes' ? 'block' : 'hidden'}>
            <div className="mb-6">
               ...
               <div className="flex flex-wrap gap-2">
                 {['GK', 'DEF', 'MID', 'ATT', 'BAL'].map(p => (<button key={p} type="button" onClick={() => applyPreset(p as any)} className="flex-1 min-w-[80px] text-xs font-medium py-2 px-3 bg-slate-700 hover:bg-slate-600 text-white rounded transition-colors border border-slate-600">{p}</button>))}
               </div>
            </div>
            <div className="space-y-6">
              {[{ name: 'pace', label: 'Ritmo' }, { name: 'shooting', label: 'Finalização' }, { name: 'passing', label: 'Passe' }, { name: 'dribbling', label: 'Drible' }, { name: 'defending', label: 'Defesa' }, { name: 'physical', label: 'Físico' }].map((attr) => (
                <div key={attr.name}>
        ...
        <div className="flex justify-end pt-4 gap-3">
          <button type="button" onClick={onCancel} className="px-4 py-2 bg-transparent text-slate-300 hover:text-white transition-colors text-sm font-medium">Cancelar</button>
          <button type="submit" disabled={isLoading} className={`flex items-center gap-2 px-6 py-2 bg-green-600 hover:bg-green-500 text-white font-medium rounded-md shadow-lg shadow-green-900/20 transition-all transform active:scale-95 ${isLoading ? 'opacity-70 cursor-wait' : ''}`}>
            {isLoading ? 'Salvando...' : (<><CheckCircle size={18} /> {initialData ? 'Salvar Alterações' : 'Preparar Convite'}</>)}
          </button>
        </div>
      </form>
    </div>
