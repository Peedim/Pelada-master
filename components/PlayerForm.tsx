...
const PlayerForm: React.FC<PlayerFormProps> = ({ initialData, onSubmit, onCancel, isLoading }) => {
  ...
  // Verifica se o utilizador atual é o MASTER (Dono)
  useEffect(() => {
  ...
  const { control, handleSubmit, watch, setValue, formState: { errors } } = useForm<PlayerFormData>({
    defaultValues: initialData ? {
      ...
      // CORREÇÃO CRÍTICA: Garante que photo_url nunca seja null para evitar erro do React
      ...
      passing: 70,
      defending: 70
      // dribbling e physical removidos
    }
  });
  ...
  // Cálculo de OVR em Tempo Real usando a fórmula oficial
  const currentOvr = useMemo(() => {
    ...
    return Math.round(calculateWeightedOvr(position as string, { 
      pace: p, shooting: s, passing: pa, defending: d 
    }));
  }, [pace, shooting, passing, defending, position]);

  // Atualiza o campo initial_ovr sempre que os atributos mudam
  useEffect(() => {
          ...
          <div className={activeTab === 'attributes' ? 'block' : 'hidden'}>
            <div className="mb-6">
               ...
               <div className="flex flex-wrap gap-2">
                 {['GK', 'DEF', 'MID', 'ATT', 'BAL'].map(p => (
                   <button 
                     key={p} 
                     type="button" 
                     onClick={() => applyPreset(p as any)} 
                     className="flex-1 min-w-[80px] text-xs font-medium py-2 px-3 bg-slate-700 hover:bg-slate-600 text-white rounded transition-colors border border-slate-600"
                   >
                     {p}
                   </button>
                 ))}
