    ...
    <div className="bg-slate-800 border-2 border-green-600/50 rounded-xl p-6 shadow-2xl relative overflow-hidden mb-8 animate-fade-in">
                ...
                {!inPenaltyMode ? (
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="flex items-center justify-between px-4 sm:px-12">
                             ...
                             <div className="flex flex-col items-center gap-2 w-1/3"><div className="text-3xl font-mono font-bold text-white">{game.penaltyShootout?.awayScore || 0}</div>{renderPenaltyDots(awayTeam.id)}</div>
                        </div>
                        {!isPenaltyWinnerDecided ? (
                            <div className="flex flex-col items-center mt-6 p-4 bg-slate-800 rounded-lg border border-slate-700 relative">
                                {game.penaltyShootout?.history.length! > 0 && (<button onClick={handleUndoPenalty} disabled={isProcessing} className="absolute top-4 right-4 text-slate-500 hover:text-white transition-colors"><RotateCcw size={18} /></button>)}
                                <p className="text-slate-400 text-sm mb-3">Cobrança de: <span className="text-white font-bold text-lg">{currentKickerTeam.name}</span></p>
                                <div className="flex gap-4">
                                    <button onClick={() => handlePenaltyKick(false)} disabled={isProcessing} className="flex flex-col items-center justify-center w-24 h-24 rounded-full bg-red-900/20 border-2 border-red-600 hover:bg-red-600 hover:text-white text-red-500 transition-all active:scale-95 disabled:opacity-50"><X size={32} /><span className="font-bold text-xs mt-1">PERDEU</span></button>
                                    <button onClick={() => handlePenaltyKick(true)} disabled={isProcessing} className="flex flex-col items-center justify-center w-24 h-24 rounded-full bg-green-900/20 border-2 border-green-600 hover:bg-green-600 hover:text-white text-green-500 transition-all active:scale-95 disabled:opacity-50"><CheckCircle size={32} /><span className="font-bold text-xs mt-1">GOL</span></button>
                                </div>
                            </div>
                        ) : (<div className="text-center py-4 bg-green-900/20 border border-green-800/50 rounded-lg animate-pulse"><h4 className="text-green-400 font-bold text-lg">Vencedor Definido!</h4><p className="text-slate-300 text-sm">{getPenaltyWinnerName()} venceu nos pênaltis.</p><p className="text-xs text-slate-500 mt-1">Pode encerrar a partida agora.</p></div>)}
                    </div>
                )}
            </div>
        )}
        ...
        {!inPenaltyMode && (
            <div className="bg-slate-900/50 rounded-lg border border-slate-700/50 p-4 mb-4">
                 <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3 flex items-center gap-2"><Zap size={12} className="text-yellow-500" /> Eventos da Partida</h4>
                 {gameGoals.length === 0 ? (<div className="text-center text-slate-500 text-sm italic py-2">Nenhum gol registrado.</div>) : (
                         ...
                         {gameGoals.map((goal) => {
                             ...
                             return (
                                 <div key={goal.id} className="flex items-center justify-between bg-slate-800 p-2 rounded border border-slate-700 hover:border-slate-600 transition-colors">
                                     <div className="flex items-center gap-3"><div className={`w-1 h-8 rounded-full ${isHomeGoal ? 'bg-blue-500' : 'bg-red-500'}`}></div><div><div className="flex items-center gap-2"><span className="text-white font-bold text-sm">{getPlayerName(goal.scorerId)}</span><span className="text-[10px] text-slate-500 bg-slate-900 px-1.5 rounded">{isHomeGoal ? homeTeam.name : awayTeam.name}</span></div>{goal.assistId && (<div className="text-xs text-slate-400">Assistência: {getPlayerName(goal.assistId)}</div>)}</div></div>
        ...
        <div className="pt-4 border-t border-slate-700 flex justify-center">
            <div className="flex items-center gap-4">
                <span className="flex items-center gap-2 text-red-500 font-bold animate-pulse text-sm uppercase tracking-widest"><div className="w-2 h-2 bg-red-500 rounded-full"></div> Em Andamento</span>
                {(!requiresPenalties || isPenaltyWinnerDecided) && (<button onClick={handleEndGameClick} className="text-xs text-slate-400 hover:text-white underline">Encerrar Partida</button>)}
            </div>
        </div>
        ...
        {isGKModalOpen && (
            <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
                <div className="bg-slate-800 border border-slate-600 rounded-xl w-full max-w-md p-6 flex flex-col max-h-[80vh]">
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="text-lg font-bold text-white flex items-center gap-2">
                            <Shirt size={20} className="text-yellow-400"/> 
                            Definir Goleiro {isSelectingFor === 'home' ? homeTeam.name : awayTeam.name}
                        </h3>
                        <button onClick={() => setIsGKModalOpen(false)} className="text-slate-400 hover:text-white"><X size={20}/></button>
                    </div>

                    <div className="flex gap-2 mb-4">
                        <button onClick={() => setGkFilterMode('GK')} className={`flex-1 py-2 text-xs font-bold rounded transition-colors ${gkFilterMode === 'GK' ? 'bg-yellow-600 text-white' : 'bg-slate-700 text-slate-400'}`}>Goleiros de Ofício</button>
                        <button onClick={() => setGkFilterMode('ALL')} className={`flex-1 py-2 text-xs font-bold rounded transition-colors ${gkFilterMode === 'ALL' ? 'bg-blue-600 text-white' : 'bg-slate-700 text-slate-400'}`}>Promover (Todos)</button>
                    </div>

                    <div className="flex-1 overflow-y-auto space-y-2 custom-scrollbar pr-2 mb-4">
                        {getFilteredPlayersForGK().map(p => (
                            <div key={p.id} onClick={() => handleSelectGK(p)} className="p-3 bg-slate-700/50 hover:bg-slate-700 rounded cursor-pointer flex items-center justify-between group">
                                <div className="flex items-center gap-3">
                                    <div className="w-8 h-8 rounded-full bg-slate-600 flex items-center justify-center text-xs font-bold text-white">{p.position.substring(0,2)}</div>
                                    <span className="text-white font-medium text-sm">{p.name}</span>
                                </div>
                                <div className="text-xs text-slate-400 font-bold">OVR {p.initial_ovr}</div>
                            </div>
                        ))}
                        {getFilteredPlayersForGK().length === 0 && <div className="text-center text-slate-500 py-4">Nenhum jogador encontrado.</div>}
                    </div>
                </div>
            </div>
        )}
        ...
        {scoringTeamId && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
                <div className="bg-slate-800 border border-slate-600 rounded-lg w-full max-w-md p-6 shadow-2xl animate-scale-up">
                    <div className="flex justify-between items-center mb-4"><h4 className="text-xl font-bold text-white">{editingGoalId ? 'Editar Gol' : `Novo Gol: ${scoringTeam?.name}`}</h4><button onClick={() => { setScoringTeamId(null); setEditingGoalId(null); }} className="text-slate-400 hover:text-white"><X size={24} /></button></div>
        ...
        {isEndGameConfirmOpen && (
             <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fade-in">
                 <div className="bg-slate-800 border border-slate-600 rounded-xl shadow-2xl w-full max-w-sm p-6 transform transition-all scale-100">
                     <div className="flex flex-col items-center text-center space-y-4"><div className="w-16 h-16 bg-yellow-900/30 rounded-full flex items-center justify-center border border-yellow-700/50"><AlertTriangle size={32} className="text-yellow-500" /></div><div><h3 className="text-xl font-bold text-white">Encerrar Partida?</h3><p className="text-slate-400 text-sm mt-2">O placar atual será registrado como final.</p>{inPenaltyMode && (<p className="text-xs text-green-400 mt-1">Resultado dos pênaltis será salvo.</p>)}</div><div className="flex gap-3 w-full mt-4"><button onClick={() => setIsEndGameConfirmOpen(false)} className="flex-1 py-2.5 bg-slate-700 hover:bg-slate-600 text-white rounded-lg font-medium transition-colors">Cancelar</button><button onClick={confirmEndGame} className="flex-1 py-2.5 bg-green-600 hover:bg-green-500 text-white rounded-lg font-bold shadow-lg shadow-green-900/20 transition-transform active:scale-95">Sim, Encerrar</button></div></div>
                 </div>
             </div>
        )}
    </div>
